...
class BaseInpainter:
    """
    Base class for all inpainter
    """
    def __init__(self, models, parameters, criterion, metric_ftns, optimizer, config):
        self.config = config
        self.logger = config.get_logger('inpainter', config['inpainter']['verbosity'])
        ...
        cfg_inpainter = config['inpainter']
        self.epochs = cfg_inpainter['epochs']
        self.save_period = cfg_inpainter['save_period']
        self.monitor = cfg_inpainter.get('monitor', 'off')
        ...
        if self.monitor == 'off':
            ...
            self.early_stop = cfg_inpainter.get('early_stop', inf)
        ...
        self.writer = TensorboardWriter(config.log_dir, self.logger, cfg_inpainter['tensorboard'])
    ...
    def _save_checkpoint(self, epoch, save_best=False):
        ...
        state = {
            ...
            'parameters': self.parameters.detach().cpu(),
